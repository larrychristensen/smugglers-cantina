...
* [`src/clj/smugglers_cantina/`](src/clj/smugglers_cantina/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/smugglers_cantina/`](src/cljs/smugglers_cantina/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/smugglers_cantina/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/smugglers_cantina/`](test/cljs/smugglers_cantina/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `smugglers-cantina` project root directory
...
in the [`src/clj/smugglers_cantina/`](src/clj/smugglers_cantina/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/smugglers_cantina/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns smugglers-cantina.example
  (:require [smugglers-cantina.config :as config])
